...
# Global vars
tarball="http://assets.nagios.com/downloads/nagiosxi/xi-latest.tar.gz"
default_services_start="ntpd mysql postgresql httpd mrtg xinetd npcd ndo2db nagios"
default_services_stop="nagios ndo2db npcd xinetd httpd postgresql mysql"
usage() {
cat <<EOF
...
Options:
	...
	-s	Service to start. May be used multiple times, services will be started in given order.
		(Default: ntpd, mysql, postgresql, httpd, npcd, mrtg, xinted, ndo2db, nagios)
