...
# Global vars
logfile="/var/log/nagioscore.log"
tarball=""
default_services_start="httpd nagios"
default_services_stop="nagios httpd"
...
update() {
	cd /tmp/
	wget "${tarball}" -o "${tar_file}" 2>&1 | tee -a "${logfile}.upgrade"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to download tarball, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	tar xvzf "${tar_file}" 2>&1 | tee -a "${logfile}.upgrade"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
	...
	./configure --with-command-group=${nagcmd_group} --with-nagios-group=${nagios_group} 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly configure, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	make 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly make, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	make install 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly make install, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	make install-init 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly make install-init, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	make install-config 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly make install-config, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	make install-commandmode 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly make install-commandmode, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi

	make install-webconf 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
		echo Failed to properly make install-webconf, exiting. 2>&1 | tee -a "${logfile}.upgrade"
		exit 5
	fi
	
	echo Finished Nagios Core updates 2>&1 | tee -a "${logfile}.upgrade"

	cd /tmp

	e 2>&1 | tee -a "${logfile}.update"
	if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
